        ...
        if (response.data.type === "HANDYMAN") {
          if (response.data.scheduleId) {
            const scheduleResponse = await ApiService.get(
              `/schedule/${response.data.scheduleId}`,
            );
          ...
          if (response.data.homeOrWorkshopId) {
            const workshopResponse = await ApiService.get(
              `/home-or-workshop/${response.data.homeOrWorkshopId}`,
            );
          ...
          if (response.data.serviceIds) {
            const serviceResponses = await Promise.all(
              response.data.serviceIds.map((id: string) =>
          ...
          const reservationResponses = await Promise.all(
            response.data.reservationIds.map(async (id: string) => {
              const reservation = await ApiService.get(`/reservation/${id}`);
              const service = await ApiService.get(
                `/service/${reservation.data.serviceId}`,
              );
              const schedule = await ApiService.get(
                `/schedule/${reservation.data.scheduleId}`,
              );
              const handyman = await ApiService.get(
                `/handyman/${schedule.data.handymanId}`,
              );
              return {
                ...reservation.data,
                service: service.data,
                handyman: handyman.data,
              };
            }),
          );
          setReservations(reservationResponses);
                        ...
                        <TabsContent value="workshop">
                          {workshop ? (
                            <Card className="mb-4">
                              ...
                              <CardContent>
                                <p>
                                  Address: {workshop.streetNumber}{" "}
                                  {workshop.streetName}, {workshop.cityName},{" "}
                                  {workshop.countryName}
                                </p>
                              </CardContent>
                            </Card>
                          ) : (
